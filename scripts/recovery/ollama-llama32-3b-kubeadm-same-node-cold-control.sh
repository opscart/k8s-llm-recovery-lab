...
# Automation controls.
RUNS="${RUNS:-1}"
AUTO_PREP_TARGET="${AUTO_PREP_TARGET:-false}"
TARGET_SSH="${TARGET_SSH:-}"
TARGET_SSH_KEY="${TARGET_SSH_KEY:-}"
TARGET_REPO="${TARGET_REPO:-~/k8s-llm-recovery-lab}"
...
prepare_target() {
  if [[ "$AUTO_PREP_TARGET" == "true" ]]; then
    [[ -n "$TARGET_SSH" ]] || fail "TARGET_SSH is required when AUTO_PREP_TARGET=true"
    require_cmd ssh

    echo
    echo "Preparing target automatically over SSH: $TARGET_SSH"

    local prep_output
    SSH_ARGS=(-o BatchMode=yes -o ConnectTimeout=10)

    if [[ -n "$TARGET_SSH_KEY" ]]; then
      SSH_ARGS+=(-i "$TARGET_SSH_KEY")
    fi

    prep_output=$(ssh "${SSH_ARGS[@]}" "$TARGET_SSH" \
      "cd $TARGET_REPO && sudo -n bash scripts/cloud/prepare-cold-target-node.sh")

    echo "$prep_output"

    echo "$prep_output" | grep -q 'model_serving_process_present=false' \
      || fail "target preparation did not confirm model_serving_process_present=false"

    echo "$prep_output" | grep -q 'drop_caches_success=true' \
      || fail "target preparation did not confirm drop_caches_success=true"
...
run_one_control() {
  ...
  [[ "$CURRENT_ROLE" == "$TARGET_ROLE" ]] || fail \
    "deployment must be in target state: role=$CURRENT_ROLE expected=$TARGET_ROLE"

  [[ "$CURRENT_PVC" == "$TARGET_PVC" ]] || fail \
    "deployment must use target PVC: pvc=$CURRENT_PVC expected=$TARGET_PVC"

  echo
  echo "Scaling Ollama to zero..."
  kubectl scale deployment -n "$NAMESPACE" "$DEPLOYMENT" --replicas=0 >/dev/null

  echo "Waiting for Ollama pod to terminate..."
  wait_for_zero_pods || fail "Ollama pod did not terminate after scale-down"
  ...
  NEW_POD=$(wait_for_ready_target_pod) || {
  ...
  wait_for_runtime || fail "Ollama runtime did not become reachable"
  T2=$(now_ms)
...
[[ "$CLIENT_NODE" != "$TARGET_NODE" ]] || fail \
  "measurement client is running on target node"
...
echo "Automatic prep : $AUTO_PREP_TARGET"

for ((i=1; i<=RUNS; i++)); do
  echo
  echo "===== CONTROL ITERATION $i OF $RUNS ====="
  run_one_control
done

echo
echo "Completed $RUNS control iteration(s)."
echo "Results:"
echo "$OUT"
