...
if ! sudo -n true 2>/dev/null; then
  echo "ERROR: passwordless sudo is required for the cold-cache experiment"
  echo "Verify sudo access before running this script."
  exit 1
fi
OUT="results/cloud-cpu/ollama/llama3.2-3b/cache/cold-cache-recovery.csv"
mkdir -p "$(dirname "$OUT")"
...
for run in $(seq 1 "$RUNS"); do
  ...
  echo "Preparing explicit cold-cache condition..."
...
echo "Waiting for model to leave runtime residency..."

MODEL_RESIDENT="true"

for attempt in $(seq 1 30); do
  MODEL_RESIDENT=$(kubectl exec -n "$NAMESPACE" "$OLD_POD" -- \
    ollama ps 2>/dev/null | grep -q "$MODEL" && echo "true" || echo "false")

  if [[ "$MODEL_RESIDENT" == "false" ]]; then
    break
  fi
...
if [[ "$MODEL_RESIDENT" != "false" ]]; then
  echo "ERROR: model remained resident after ollama stop"
  exit 1
fi

echo "Dropping host filesystem/page caches..."
...
if sudo sh -c 'echo 3 > /proc/sys/vm/drop_caches'; then
  ...
  DROP_CACHES_SUCCESS="false"
fi

if [[ "$DROP_CACHES_SUCCESS" != "true" ]]; then
  echo "ERROR: failed to drop host caches"
  exit 1
fi

CACHED_AFTER_DROP=$(awk '/^Cached:/ {print $2}' /proc/meminfo)

echo "Cold precondition confirmed"
echo "  Model resident    : $MODEL_RESIDENT"
echo "  Cached before     : ${CACHED_BEFORE} kB"
echo "  Cached after drop : ${CACHED_AFTER_DROP} kB"
echo "  drop_caches       : $DROP_CACHES_SUCCESS"
  ...
  echo "$run,cold,$MODEL_RESIDENT,$CACHED_BEFORE,$CACHED_AFTER_DROP,$DROP_CACHES_SUCCESS,$T0,$T1,$T2,$T3,$T4,$POD_RECOVERY,$RUNTIME_RECOVERY,$READY_TO_RUNTIME,$FUNCTIONAL_RECOVERY,$READY_TO_INFERENCE,$REQUEST_WALL,$HTTP_CODE,$TOTAL_MS,$LOAD_MS,$PROMPT_MS,$EVAL_MS,$MEMORY_CURRENT" \
  >> "$OUT"
  if [[ "$run" -lt "$RUNS" ]]; then
